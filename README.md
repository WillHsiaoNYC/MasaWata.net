...
Each app directory is a self-contained, multi-language marketing site generated by its own `build.py`.

## Sites

- **Home** (`/`): Studio hub — links to every app, with `Organization` / `WebSite` / `ItemList` structured data.
- **Privacy Policy** (`/privacy-policy.html`): Shared privacy policy for the apps. This is the canonical privacy URL every app links to.
